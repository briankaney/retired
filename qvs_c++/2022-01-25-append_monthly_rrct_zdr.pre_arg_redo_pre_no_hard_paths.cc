...
//--------------------------------------------------------------------------------------
  ...
  if(argc<2) {
    cout<<endl<<endl;
    cout<<"append_monthly_rrct_zdr year month"<<endl<<endl;
    return -1;
    }
...
//--------------------------------------------------------------------------------------
  ...
  sprintf(bin_file,"/web_data/qvs_data/rrct/%d/%02d/RRCT_ZDR.%d%02d.gz",year,month,year,month);
...
//20180701.000500 1 24.64 25.51 28.99 30.07 4243
  ...
  for(int i=0;i<dat->number_list;++i) {
    sprintf(rrct_zdr_file,"/web_data/qvs_data/rrct/%d/%02d/zdr/%s_%d%02d_ZDR_Comparison",year,month,dat->list_names[i],year,month);
 
    if( (inf=fopen(rrct_zdr_file,"r"))==NULL ) {
      if(DEBUG==1)  cout<<"Failure opening input file: "<<rrct_zdr_file<<endl;
      continue;
      }

    while(fgets(buffer,MAX_INPUT_LINE_LEN-2,inf)!=NULL) {  
      sscanf(buffer,"%s %d %f %f %d",timestamp,&read_min_diff,&read_zdr1,&read_zdr2,&read_bin_count);
      ...
      dat->data[total_offset] = (short int)(read_zdr1*dat->variable_scales[1]);

      total_offset = var_offset[2] + list_offset*i + (read_day-1)*288 + read_hour*12 + read_minute/5;
      dat->data[total_offset] = (short int)(read_zdr2*dat->variable_scales[2]);
